...
public class Gamma {
    ...
    public static final int TOTAL_RAMP_VALUES_COUNT = 768;
    public static final int SINGLE_RAMP_CHANNEL_VALUES_COUNT = 256;
    public static final int TOTAL_COLORS_COUNT = 3;
    ...
    private final WORD[] gammaRamp = new WORD[TOTAL_RAMP_VALUES_COUNT];
    ...
    void invertGammaRamp(Channel channel) {
        System.out.println(channel + ": " + invertedChannels[channel.index] + " :: " + !invertedChannels[channel.index]);
    ...
    void reinitializeGammaRamp() {
        int[][] newGammaRamp = new int[TOTAL_COLORS_COUNT][SINGLE_RAMP_CHANNEL_VALUES_COUNT];
        for (int y = 0; y < newGammaRamp.length; y++) {
            if (invertedChannels[y]) {
                for (int x = 0; x < newGammaRamp[0].length; x++) {
                    ...
                    newGammaRamp[y][SINGLE_RAMP_CHANNEL_VALUES_COUNT - x - 1] = newGammaRampValue;
                }
            } else {
                for (int x = 0; x < newGammaRamp[0].length; x++) {
                    int newGammaRampValue = calculateRampValue(newGammaRamp, x, y);
    ...
    private int calculateRampValue(int[][] newGammaRamp, int x, int y) {
        double rampValue = Math.pow((double) x / SINGLE_RAMP_CHANNEL_VALUES_COUNT, gamma[y]) * MAX_WORD;
    ...
    int[][] getGammaRamp() {
        getDeviceGammaRamp();
        int[][] rampValues = new int[TOTAL_COLORS_COUNT][SINGLE_RAMP_CHANNEL_VALUES_COUNT];
        for (int i = 0; i < TOTAL_RAMP_VALUES_COUNT; i++) {
            rampValues[i / SINGLE_RAMP_CHANNEL_VALUES_COUNT][i % SINGLE_RAMP_CHANNEL_VALUES_COUNT] = gammaRamp[i].intValue();
        }
        return rampValues;
    }
    ...
    double[][] getNormalizedGammaRamp() {
        getDeviceGammaRamp();
        double[][] scaledGammaRamp = new double[TOTAL_COLORS_COUNT][SINGLE_RAMP_CHANNEL_VALUES_COUNT];
        int[][] gammaRamp = getGammaRamp();
        for (int y = 0; y < TOTAL_COLORS_COUNT; y++) {
            for (int x = 0; x < SINGLE_RAMP_CHANNEL_VALUES_COUNT; x++) {
